...
router.get('/floor-history', async (req, res) => {
    try {
        const days = Math.min(parseInt(req.query.days) || 30, 365);
        const data = await (0, redis_1.cached)(`market:floor-history:${days}`, 60 * 5, async () => {
            const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
            const snapshots = await prisma_1.prisma.historicalSnapshot.findMany({
            ...
            const history = snapshots
                ...
                floor: s.floorPriceEth,
            }));
            // Ensure we have at least current floor
            if (history.length === 0) {
                const floor = await marketDataProvider_1.marketDataProvider.getFloor().catch(() => null);
...
router.get('/trait-demand', async (req, res) => {
    try {
        const data = await (0, redis_1.cached)('market:trait-demand', 60 * 5, async () => {
            const now = new Date();
            ...
            async function getTraitCounts(tokenIds) {
