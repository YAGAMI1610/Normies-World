...
import type { WhaleSimilarityResult } from "@normies-alpha/shared-types";

interface WalletProfile {
  rarityPreference: Record<string, Record<string, number>>; // normalized 0-1 per category
...
async function buildWalletProfile(walletAddress: string): Promise<WalletProfile | null> {
  ...
  if (!wallet || wallet.ownerships.length === 0) return null;
...
function traitCosineSimilarity(
...
export async function computeSimilarity(
  ...
  whaleAddress: string
): Promise<WhaleSimilarityResult | null> {
  const [userProfile, whaleProfile] = await Promise.all([
    buildWalletProfile(userWalletAddress),
    buildWalletProfile(whaleAddress),
  ]);

  if (!userProfile || !whaleProfile) return null;

  const traitPreference = traitCosineSimilarity(
    userProfile.rarityPreference,
    whaleProfile.rarityPreference
  );
  ...
  const holdDuration = closeness(
    userProfile.avgHoldDurationDays,
    whaleProfile.avgHoldDurationDays,
    365
  );
  ...
  for (const category of categories) {
    ...
    if (userTop && whaleTop && userTop === whaleTop) overlapCount++;
  }
...
function topValue(dist?: Record<string, number>): string | null {
  if (!dist) return null;
...
export async function findMostSimilarWhales(
  userWalletAddress: string,
  limit = 5
