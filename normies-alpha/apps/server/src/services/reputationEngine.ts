...
import type { BadgeType, ReputationProfile } from "@normies-alpha/shared-types";
...
export async function recalculateReputation(userId: string): Promise<ReputationProfile | null> {
  ...
  // --- Badges ---
  const badgeChecks: { type: BadgeType; earned: boolean }[] = [
  ...
  const badges = await prisma.badge.findMany({ where: { reputationId: reputation.id } });
  ...
  return {
    ...
    xp,
    xpToNextLevel,
    badges: badges.map((b) => ({ type: b.type as BadgeType, awardedAt: b.awardedAt.toISOString() })),
  };
}

export async function getReputationByAddress(address: string): Promise<ReputationProfile | null> {
