...
exports.WHALE_SCORE_THRESHOLD = exports.WHALE_HOLDING_THRESHOLD = void 0;
exports.recalculateWhale = recalculateWhale;
exports.recalculateAllWhales = recalculateAllWhales;
exports.getWhaleLeaderboard = getWhaleLeaderboard;
const prisma_1 = require("../lib/prisma");
const marketDataProvider_1 = require("./marketDataProvider");
exports.WHALE_HOLDING_THRESHOLD = 15;
exports.WHALE_SCORE_THRESHOLD = 70;
async function computeWalletStats(whaleAddress) {
    ...
    const holdDurations = wallet.ownerships.map((o) => (now - o.acquiredAt.getTime()) / (1000 * 60 * 60 * 24));
    const avgHoldDurationDays = holdDurations.reduce((a, b) => a + b, 0) / holdDurations.length;
    ...
    const sellSales = await prisma_1.prisma.sale.findMany({
    ...
    const recentAcquisitions = await prisma_1.prisma.normieOwnership.count({
        where: {
            walletId: wallet.id,
    ...
    const whaleScore = profitability * 0.35 +
        rarityQuality * 0.25 +
        consistency * 0.2 +
...
async function recalculateWhale(whaleAddress) {
    const stats = await computeWalletStats(whaleAddress);
    if (!stats)
        return null;
    const isWhale = stats.holdingsCount >= exports.WHALE_HOLDING_THRESHOLD ||
        stats.whaleScore >= exports.WHALE_SCORE_THRESHOLD;
    await prisma_1.prisma.wallet.update({
        where: { address: stats.whaleAddress },
        data: { isWhale, whaleScore: stats.whaleScore },
    });
    if (isWhale) {
        await prisma_1.prisma.whale.upsert({
        ...
        await prisma_1.prisma.whale.deleteMany({ where: { whaleAddress: stats.whaleAddress } });
    }
    return stats;
}
/** Recompute whale stats for every wallet holding at least 1 Normie. Run as a periodic job. */
async function recalculateAllWhales() {
    ...
    for (const w of wallets) {
        ...
        if (result && (result.holdingsCount >= exports.WHALE_HOLDING_THRESHOLD || result.whaleScore >= exports.WHALE_SCORE_THRESHOLD)) {
            whaleCount++;
        }
    }
    return whaleCount;
}
async function getWhaleLeaderboard(limit = 20) {
    return prisma_1.prisma.whale.findMany({
