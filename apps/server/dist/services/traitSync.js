...
exports.syncAllTraits = syncAllTraits;
exports.computeRarityScores = computeRarityScores;
...
async function syncAllTraits(force = false) {
    console.log("[traitSync] starting trait sync...");
    const alreadySynced = force
        ? new Set()
        : new Set((await prisma_1.prisma.normie.findMany({
            where: { traits: { some: {} } },
            select: { tokenId: true },
        })).map((n) => n.tokenId));
    for (let tokenId = 0; tokenId < shared_types_1.TOTAL_SUPPLY; tokenId++) {
        if (alreadySynced.has(tokenId))
            continue;
        try {
            ...
            const pixelCountAttr = metadata?.attributes.find((a) => a.trait_type === "Pixel Count");
            ...
            if (tokenId % 250 === 0) {
                console.log(`[traitSync] synced token ${tokenId}/${shared_types_1.TOTAL_SUPPLY}`);
            }
        }
        catch (err) {
 ...
 */
async function computeRarityScores() {
    const allTraits = await prisma_1.prisma.trait.findMany({
    ...
    for (let i = 0; i < scores.length; i += BATCH) {
        ...
        await prisma_1.prisma.$transaction(batch.map((s, idx) => prisma_1.prisma.normie.update({
